...
export default function AdminBlogList() {
  ...
  const togglePublish = trpc.blog.update.useMutation({
    onSuccess: () => { utils.blog.adminList.invalidate(); toast.success("Post updated"); },
    onError: () => toast.error("Failed to update post"),
  });

  const deletePost = trpc.blog.delete.useMutation({
    onSuccess: () => { utils.blog.adminList.invalidate(); toast.success("Post deleted"); },
      ...
      <div className="max-w-4xl space-y-5">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-sm">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "oklch(0.52 0.015 250)" }} />
            <input
              type="text"
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 rounded-xl border" style={{ borderColor: "oklch(1 0 0 / 8%)", background: "oklch(0.10 0.012 265)" }}>
            <BookOpenIcon />
            <p className="text-sm mt-3 mb-4" style={{ color: "oklch(0.52 0.015 250)", fontFamily: "'Inter', sans-serif" }}>
            ...
            {posts.map((post) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate" style={{ color: "oklch(0.88 0.005 240)", fontFamily: "'Inter', sans-serif" }}>
                    {post.title}
                  </div>
                  <div className="flex items-center gap-3 mt-0.5">
                    <span className="text-xs" style={{ color: "oklch(0.52 0.015 250)", fontFamily: "'JetBrains Mono', monospace" }}>
                      {post.category}
                    </span>
...
function BookOpenIcon() {
  return (
    <div className="w-10 h-10 rounded-xl mx-auto flex items-center justify-center" style={{ background: "oklch(0.82 0.15 200 / 10%)" }}>
