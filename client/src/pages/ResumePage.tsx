        ...
        {experience.map((job, i) => (
          <div key={i} className="relative pl-12">
            ...
            <div
              ...
              style={{
                background: job.current ? "oklch(0.82 0.15 200 / 15%)" : "oklch(0.16 0.012 265)",
                border: `1px solid ${job.current ? "oklch(0.82 0.15 200 / 50%)" : "oklch(1 0 0 / 10%)"}`,
              }}
            >
              <Briefcase size={13} style={{ color: job.current ? "oklch(0.82 0.15 200)" : "oklch(0.52 0.015 250)" }} />
            </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-sm font-medium" style={{ fontFamily: "'Inter', sans-serif", color: "oklch(0.82 0.15 200)" }}>
                      {job.company}
                    </span>
                  </div>
                </div>
              ...
              <ul className="space-y-1.5">
                {job.highlights.map((h, j) => (
                  <li key={j} className="flex items-start gap-2 text-sm"
                    ...
                    <span className="flex-shrink-0 mt-1.5 w-1 h-1 rounded-full" style={{ background: "oklch(0.82 0.15 200 / 60%)" }} />
                    {h}
                  </li>
                ))}
              </ul>
...
function Education() {
  return (
    <div className="gradient-border rounded-xl p-6">
      ...
      <p className="text-sm font-medium mb-1" style={{ fontFamily: "'Inter', sans-serif", color: "oklch(0.82 0.15 200)" }}>
